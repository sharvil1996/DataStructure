 ...
 *         or right subtree cannot be greater than one.
 * 
 *         There are four different use cases to insert into AVL tree left left
 *         - needs ones right rotation left right - needs one left and one right
 *         rotation right left - needs one right and one left rotation right
 *         right - needs one left rotation
 * 
...
*/
		...
		System.out.println();
		tt.preOrder(root);
		TreeTraversalLevelByLevel t = new TreeTraversalLevelByLevel();
		System.out.println();
		t.levelByLevelTwoQueue(root);
